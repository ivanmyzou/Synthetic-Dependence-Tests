...
for (n in sample_sizes) {
  d <- cbind(read.fst(sprintf("1. Threshold/threshold/%s/thres.fst", n)), n,
             read.fst(sprintf("1. Threshold/threshold/%s/thres_image.fst", n)))
...
image_thresholds %>% saveRDS('4. Estimated Thresholds/image_thresholds.RDS')
score_thresholds %>% saveRDS('4. Estimated Thresholds/score_thresholds')
combined_thresholds %>% saveRDS('4. Estimated Thresholds/combined_thresholds')
...
other_thresholds <- foreach(n = sample_sizes, .combine = 'rbind') %do% {
  d <- cbind(read.fst(sprintf("1. Threshold/threshold/%s/thres.fst", n)))
...
other_thresholds %>% saveRDS('4. Estimated Thresholds/other_thresholds.RDS')
