...
sample_sizes_remain <- as.character(sample_sizes) %>% setdiff(list.dirs('./dependent tests/', full.names = FALSE))
...
for (sample_size in sample_sizes_remain) {
  ...
  cat('writing files ...\n')
  dir.create(paste0('./dependent tests/', sample_size)) #create folder

  write.fst(Xind, paste0('./dependent tests/', sample_size, '/', 'ind.fst'))
  write.fst(Ximage, paste0('./dependent tests/', sample_size, '/', 'ind_image.fst'))
}
